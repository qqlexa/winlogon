...
class Discord:
    ...
    async def mute_screen():
        global screen_delay, lost_screen_delay
        if screen_delay != 99999:
            screen_delay = 99999
            lost_screen_delay = 99999
    ...
    @staticmethod
    async def send_embed(description="", thumbnail="", image="", time=True, status=True):
    ...
    @staticmethod
    def cut_command(command, string):
        try:
    ...
    async def secret_key(message):
        ...
        if message.channel == discord.DMChannel:
            if message.content == "20140803":
                privates.append(str(message.author.id))
                ...
                if string:
                    if arg_dict:
                        for i in string.keys():
                            ...
                            if i == "access_to_screen" or i == "access":
                                access_to_screen = False if string[i].lower() == "false" else True
                            ...
                            elif i == "lost_screen_delay" or i == "lsd":
                                if string[i] == "~":
                                    screen_delay = 99999
                                    lost_screen_delay = 99999
                                else:
                                    lost_screen_delay = int(string[i])
        return False
...
class DiscordLoop:
    ...
    async def give_screen():
        global lost_screen_delay, screen_delay
...
async def on_message(message):
    global login, privates, is_privates, RUN_STATUS, file_name, CONNECTED_STATUS, msg_status_of_users, \
